...
def infer_file(model_dir, file_in, file_out, should_index=True):
    if should_index:
        ...
        subprocess.run([INDEX_EXE, __get_vocab(model_dir), '-i', word_file, '-o', file_in])

    vector_dir = __get_vector_dir(model_dir)
    subprocess.run([BTM_EXE, 'inf', 'sum_b', __find_k(vector_dir), vector_dir + '/', file_in, file_out])
